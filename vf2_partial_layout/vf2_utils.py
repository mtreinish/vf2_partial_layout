...
# This module was forked from qiskit-terra's qiskit/transpiler/passes/layout/vf2_utils.py
# to temporarily workaround a bug. This will be removed in a future release
...
def build_interaction_graph(dag, strict_direction=True):
    ...
    im_graph = (
        PyDiGraph(multigraph=False) if strict_direction else PyGraph(multigraph=False)
    )
    im_graph_node_map = {}
...
def score_layout(
    avg_error_map, layout, bit_map, reverse_bit_map, im_graph, strict_direction=False
):
...
def build_average_error_map(target, properties, coupling_map):
    ...
    if target is not None:
        ...
        for qubit in range(num_qubits):
            avg_map[(qubit,)] = (
                coupling_map.graph.out_degree(qubit)
                + coupling_map.graph.in_degree(qubit)
            ) / num_qubits
